...
class testdata:
    ...
    def get_loaders(self, parse_patches=False):  #, validation='test'):
        print("=> evaluating on test set...")
        # train_dataset = testDataset(dir=os.path.join(self.config.data.data_dir, 'data'),
        #                                 n=self.config.training.patch_n,
        #                                 patch_size=self.config.data.image_size,
        #                                 transforms=self.transforms,
        #                                 filelist=None,
        #                                 parse_patches=parse_patches)
        # val_dataset = testDataset(dir=os.path.join(self.config.data.data_dir, 'test_data', 'RESIDE_6K', 'hazy_400'),#'SOTS_Test', 'valid_indoor', 'input_624_464'),
        #                               # n=self.config.training.patch_n,
        #                               # patch_size=self.config.data.image_size,
        #                               transforms=self.transforms,
        #                               filelist=None,
        ...
        val_dataset = testDataset(dir=os.path.join(self.config.data.data_dir, 'test_data', 'NH_HAZE', 'hazy'),
                                  # 'SOTS_Test', 'valid_indoor', 'input_624_464'),
                                  # n=self.config.training.patch_n,
                                  # patch_size=self.config.data.image_size,
                                  transforms=self.transforms,
                                  filelist=None,
        ...
        val_dataset = testDataset(
            dir=os.path.join(self.config.data.data_dir, 'test_data', 'refine'),
            n=self.config.training.patch_n,
            patch_size=self.config.data.image_size,
            transforms=self.transforms,
            filelist=None,
...
class testDataset(torch.utils.data.Dataset):
    def __init__(self, dir, transforms, filelist=None, parse_patches=True):
        ...
        if filelist is None:
            ...
            # gt_names += [os.path.join(os.path.join(test_dir, 'test-clear'), i.split('/')[-1].split('_')[0] + '.png') for i in images]
        ...
        #         input_names = [i.strip() for i in contents]
                # gt_names = [i.strip().replace('hazy', 'hazy') for i in input_names]
        self.dir = dir
    ...
    def get_images(self, index):
        ...
        if input_name.endswith(".jpg"):
            img_id = input_name.split('/')[-1].split('.jpg')[0] # haze4k
        else:
            img_id = input_name.split('/')[-1].split('.png')[0]
        # img_id = input_name.split('/')[-1].split('.')[0]  # sots_indoor
        # img_id = input_name.split('/')[-1].split('_')[0]  # sots_outdoor
        ...
        #     outputs = [torch.cat([self.transforms(input_img[i])], dim=0)
