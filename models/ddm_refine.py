...
from models.LFRM import Refine
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # e = torch.randn_like(input_LL)  # 在变换后的结果上进行扩散
...
class DenoisingDiffusion_refine(object):
    def __init__(self, args, config):
        super().__init__()
        self.args = args
        self.config = config
    ...
    def load_ddm_ckpt(self, load_path, ema=False):
        ...
        self.model.load_state_dict(checkpoint['state_dict'], strict=False)  # 首次导入为False
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.step % 100 == 0:
                    ...
                    print("epoch:{}, step:{},l1_loss:{:.4f}, ssim_loss:{:.4f}".format(epoch, self.step, l1_loss.item(), ssim_loss.item()))
                ...
                if self.step % self.config.training.validation_freq == 0 and self.step != 0:
                    ...
                    utils.logging.save_checkpoint({
                                                  ...
                                                  filename=os.path.join(self.config.data.ckpt_dir, 'refine_iter_{}'.format(self.step)))
            self.scheduler.step()


    def sample_validation_patches(self, val_loader, step):
        image_folder = os.path.join(self.args.image_folder, 'val_refine_iter{}'.format(self.step))
        self.model.eval()
        with torch.no_grad():
